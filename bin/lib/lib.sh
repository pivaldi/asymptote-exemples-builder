...
## @brief The OLConfig Library is a small Bash script that acts as a library to configure OVYA projects
## in bash scripts.
## @copyright New BSD
## @version 0.1.0
...
. $(dirname "${BASH_SOURCE[0]}")/ol-bash/lib/bsfl.bash || exit 1
...
function _done() {
  ...
  echo "$($GREEN) $($DEFAULT_CLR) ${__DOING_MSG} : $($GREEN)$DONE$($DEFAULT_CLR)"
  echo
}
