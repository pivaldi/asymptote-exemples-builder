...
generate_asy_cmd := $(current_dir)/bin/generate-asy.sh
generate_md_hexo_cmd := $(current_dir)/bin/generate-md-hexo.sh

.PHONY: all
all: install asy xmls html md-hexo
...
.PHONY: asy
asy: install ## Build all the Asymptote codes' example only if the example is newer than the generated picture.
	$(generate_asy_cmd)

.PHONY: xmls
xmls: asy ## Generate xml files from asy build files. XML files are the base files that can be tread by xsl files to generate various format.
	$(generate_xmls_cmd)
...
.PHONY: asy-clean
asy-clean: ## Clean the compiled Asymptote pictures and pdf on the asy build dir.
...
.PHONY: deploy-hexo
deploy-hexo: asy xmls md-hexo sync-hexo ## Usage for blog.piprime.fr only. Synchronize to my personal Hexo blog.
	$(current_dir)/bin/sync-to-hexo-blog.sh
